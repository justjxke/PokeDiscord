...
describe("handleTrackCompletion", () => {
  ...
  test("restarts the current track instead of advancing the queue when the end reason is replaced", async () => {
    const playedTracks: Array<{ track: { encoded: string }; position?: number }> = [];
    const session = {
      destroyed: false,
      guildId: "guild-1",
    ...
    await handleTrackCompletion(new Map([["guild-1", session as never]]), session as never, async () => {}, "replaced", "encoded-current");
